...
def process_frame(frame, black_and_white=False, dither=False, dither_amount=100, add_noise_amount=0, contrast_amount=100, saturation_amount=100):
    ...
    if black_and_white:
        original_image = original_image.convert('L').convert('RGB')  # Convert to grayscale and back to RGB
...
video_to_mp4(
    ...
    output_video_path="/Users/agi/Dropbox/ComfyUI_Output/2024-07-21/dither/0720_9843576_00002_bw_95_b.mp4",
    ...
    dither_amount=95,  # set dithering amount to 95%
    add_noise_amount=5,  # add noise to each frame for lossy effect
    contrast_amount=100,  # adjust contrast to make dithering pop (0-100 scale)
